/**
 * AirPulse Main Coordinator & Controller
...
const State = {
  ...
  waqiToken: '8351afd9c57845fd2ada91885f218c3c84856d50afa1fd8f7a0b4551d17d23f0',
  currentLocation: {
    name: 'London',
    lat: 51.5074,
    lng: -0.1278,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Test default token in background
  testTokenConnection();
...
function loadLocalStorage() {
  ...
  const savedToken = localStorage.getItem('airpulse-token');
...
function saveStateToStorage() {
  ...
  localStorage.setItem('airpulse-token', State.waqiToken);
...
async function testTokenConnection() {
  ...
  if (ok) {
    ...
    text.textContent = 'Token authentication failed!';
    text.style.color = 'var(--aqi-unhealthy)';
    showToast('WAQI API Key validation failed. Gracefully falling back.', 'error');
  }
}
  ...
  try {
    ...
    if (State.provider === 'waqi') {
      try {
        ...
        // Safe Fallback to Open-Meteo in case WAQI token fails or errors out
        console.warn('WAQI failed, falling back to Open-Meteo:', err.message);
        showToast('WAQI API unavailable or token invalid. Falling back to Open-Meteo.', 'info');
...
function updateDashboardUI(data) {
  ...
  // Set AQI colors dynamically using HSL maps
  const aqiColors = {
  ...
  const maxVal = 300; // Cap at 300 for visualization
  ...
  // Update Pollutant Profile Grid values & bars
  ...
  // Load interactive map station coordinates
  MapModule.initMap(
    'leaflet-map',
    data.lat,
    data.lng,
...
function updatePollutantCard(id, val, good, mod, sens, unh, very) {
  ...
  // Color mapping based on concentration levels
...
function updateHealthAdvisory(aqi) {
  ...
  if (aqi <= 50) {
    ...
    outExposure.textContent = 'Avoid prolonged exposure outdoors if you are sensitive to dust/smoke.';
...
function bindEvents() {
  ...
  document.getElementById('btn-settings-drawer').addEventListener('click', () => {
    ...
    MapModule.invalidateSize(); // Ensure leaflet map updates coordinates correctly
  });
  ...
  document.getElementById('provider-waqi').addEventListener('click', () => {
    ...
    refreshData();
    testTokenConnection();
    showToast('Provider switched to WAQI API', 'success');
  });
  ...
  // Hide autocomplete if clicking elsewhere
  ...
  // Chart view switches
  document.getElementById('btn-chart-hourly').addEventListener('click', (e) => {
...
function renderAutocompleteDropdown(results) {
  ...
  results.forEach(loc => {
    ...
    // Click handler for geocoded location
...
function renderFavoritesList() {
  ...
  State.favorites.forEach((fav, index) => {
    ...
    // Stop event bubble so clicking the item itself triggers navigation, not deletion!
    ...
    // Clicking active favorite jumps to location coordinates
    item.addEventListener('click', () => {
